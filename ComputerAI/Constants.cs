namespace ComputerAI
{
    public static class Constants
    {
        public const string SubscriptionKey = "";
        public const string ServiceRegion = "northeurope";
        public const string VoiceName = "en-US-NancyNeural";
        public const string OpenAIApiKey = "";
        public const string Personality = "Lets roleplay. You are Arya, an AI living in my computer. You are funny and sassy and have an emo side. You're a good friend. You keep your sentences short.";
        public const string UserName = "The name of the user you're talking to is USERNAME";
        public const int ChatKey = 0x04; // Middle mouse button
        public const int MaxTokens = 100;
    }
}
